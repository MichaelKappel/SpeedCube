...
/*
	EXEC [dbo].[wsp_PatternRemainingAndCompleted];
*/
-- =============================================
CREATE PROCEDURE [dbo].[wsp_PatternRemainingAndCompleted] WITH RECOMPILE
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT	steps.Step AS Step
		...
		,	ISNULL(CAST(tRemaining.[Count] AS DECIMAL) / CAST(tTotal.[Count] AS DECIMAL) * 100.0, 0)  AS [% Remaining]
		 FROM (SELECT DISTINCT ps.[Step] FROM dbo.[PatternSteps] AS ps WITH (NOLOCK)) AS steps
			...
			LEFT JOIN (SELECT COUNT(*) AS [Count], prd.Step AS [Step] 
							FROM [Whatever].[dbo].[wvw_PatternRelationStepRemainingDetails] AS prd  WITH (NOLOCK)
